"""
scripts/run_true.py
One-click runner for the true IEEE 34-bus OPF case (no Jupyter required).

Usage:
    python scripts/run_true.py
    python scripts/run_true.py --data "data/900131_*.csv" --epochs 100
"""
...
from src.opf_true    import solve_opf, P_LOAD_TOTAL, PV_BUS, WIND_BUS
from src.monte_carlo import run_mc, summarize
from src             import utils


def load_config(path="configs/true_ieee34.yaml"):
    with open(path) as f:
...
def main(args):
    ...
    print("=== True IEEE 34-bus OPF ===")
    print(f"P_LOAD_TOTAL = {P_LOAD_TOTAL:.3f} MVA")
    print(f"PV   (Bus {PV_BUS}): mu={pv_mu:.3f}  sigma={sigma_pv:.3f}")
    print(f"Wind (Bus {WIND_BUS}): mu={wind_mu:.3f}  sigma={sigma_wind:.3f}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run True IEEE 34-bus OPF")
    parser.add_argument("--config", default="configs/true_ieee34.yaml")
